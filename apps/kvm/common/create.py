...
class VmCreate(KvmBase):
    ...
    def create_instance(self, name, memory, vcpu, disk, uuid, mac, arch='x86_64', machine='pc', media=None):
        """
        Create VM function
        """
        ...
        for i in media:
            ...
            xml += f"""
                    <disk type='file' device='{i.get('type')}'>
                      <source file='{i.get('file')}'/>
                      <target dev='{dev}' bus='{bus}'/>
                      {'<driver name="qemu" type="qcow2"/>' if i.get('type') == 'disk' else '<driver name="qemu" type="raw"/>'}
                      {'<readonly/>' if i.get('type') == 'floppy' else '' }
                    </disk>
                    """
        xml += f"""
