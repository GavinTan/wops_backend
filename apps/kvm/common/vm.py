...
from .storage import KvmStoragePool, KvmStorageBase
import time


class wvmInstances(KvmBase):
    ...
    def get_instance_memory(self, name):
        inst = self.get_instance(name)
        mem = utils.get_xml_path(inst.XMLDesc(0), "/domain/currentMemory")
        return int(mem) // 1024

    def get_instance_vcpu(self, name):
        inst = self.get_instance(name)
        cur_vcpu = utils.get_xml_path(inst.XMLDesc(0), "/domain/vcpu/@current")
        if cur_vcpu:
            vcpu = cur_vcpu
        else:
            vcpu = utils.get_xml_path(inst.XMLDesc(0), "/domain/vcpu")
        return vcpu
    ...
    def graphics_type(self, name):
        inst = self.get_instance(name)
        console_type = utils.get_xml_path(inst.XMLDesc(0), "/domain/devices/graphics/@type")
    ...
    def graphics_listen(self, name):
        inst = self.get_instance(name)
        listen_addr = utils.get_xml_path(inst.XMLDesc(0), "/domain/devices/graphics/@listen")
        if listen_addr is None:
            listen_addr = utils.get_xml_path(inst.XMLDesc(0), "/domain/devices/graphics/listen/@address")
            if listen_addr is None:
                return "None"
        return listen_addr
    ...
    def graphics_port(self, name):
        inst = self.get_instance(name)
        console_port = utils.get_xml_path(inst.XMLDesc(0), "/domain/devices/graphics/@port")
    ...
    def domain_name(self, name):
        inst = self.get_instance(name)
        domname = utils.get_xml_path(inst.XMLDesc(0), "/domain/name")
        if domname is None:
            return "NoName"
        return domname
    ...
    def graphics_passwd(self, name):
        inst = self.get_instance(name)
        password = utils.get_xml_path(inst.XMLDesc(VIR_DOMAIN_XML_SECURE), "/domain/devices/graphics/@passwd")
        if password is None:
            return "None"
        return password


class KvmVmBase(KvmBase):
    def __init__(self, host, login, passwd, conn, vname, conn_pool=False):
        KvmBase.__init__(self, host, login, passwd, conn, conn_pool)
        self._ip_cache = None
    ...
    def get_vcpu(self):
        vcpu = utils.get_xml_path(self._XMLDesc(0), "/domain/vcpu")
        return int(vcpu)

    def get_cur_vcpu(self):
        cur_vcpu = utils.get_xml_path(self._XMLDesc(0), "/domain/vcpu/@current")
        if cur_vcpu:
            return int(cur_vcpu)
        else:
    ...
    def get_arch(self):
        return utils.get_xml_path(self._XMLDesc(0), "/domain/os/type/@arch")

    def get_machine_type(self):
        return utils.get_xml_path(self._XMLDesc(0), "/domain/os/type/@machine")

    def get_dom_emulator(self):
        return utils.get_xml_path(self._XMLDesc(0), "/domain/devices/emulator")

    def get_nvram(self):
        return utils.get_xml_path(self._XMLDesc(0), "/domain/os/nvram")

    def get_loader(self):
        xml = self._XMLDesc(0)
        loader = utils.get_xml_path(xml, "/domain/os/loader")
        loader_type = utils.get_xml_path(xml, "/domain/os/loader/@type")
        readonly = utils.get_xml_path(xml, "/domain/os/loader/@readonly")
    ...
    def get_memory(self):
        mem = utils.get_xml_path(self._XMLDesc(0), "/domain/memory")
        return int(mem) * 1024

    def get_cur_memory(self):
        mem = utils.get_xml_path(self._XMLDesc(0), "/domain/currentMemory")
        return int(mem) * 1024

    def get_title(self):
        title = utils.get_xml_path(self._XMLDesc(0), "/domain/title")
    ...
    def get_filterrefs(self):
        ...
        return utils.get_xml_path(self._XMLDesc(0), func=filterrefs)

    def get_description(self):
        description = utils.get_xml_path(self._XMLDesc(0), "/domain/description")
    ...
    def get_net_devices(self):
        ...
        return utils.get_xml_path(self._XMLDesc(0), func=networks)
    ...
    def get_disk_devices(self):
            ...
            for disk in doc.xpath('/domain/devices/disk'):
                ...
                try:
                    ...
                    try:
                        ...
                        self.get_volume_by_path(src_file)
                    ...
                    try:
                        ...
                        disk_size = utils.convert_bytes(vol.info()[1]) if vol.info()[1] else vol.info()[1]
                        used_size = utils.convert_bytes(vol.info()[2]) if vol.info()[2] else vol.info()[2]
        ...
        return utils.get_xml_path(self._XMLDesc(0), func=disks)
    ...
    def get_bootmenu(self):
        menu = utils.get_xml_path(self._XMLDesc(0), "/domain/os/bootmenu/@enable")
    ...
    def get_console_listen_addr(self):
        listen_addr = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics/@listen")
        if listen_addr is None:
            listen_addr = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics/listen/@address")
            if listen_addr is None:
                return "127.0.0.1"
        return listen_addr
    ...
    def get_console_socket(self):
        socket = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics/@socket")
        return socket

    def get_console_type(self):
        console_type = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics/@type")
        return console_type
    ...
    def get_console_port(self, console_type=None):
        if console_type is None:
            console_type = self.get_console_type()
        port = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics[@type='%s']/@port" % console_type)
        return port

    def get_console_websocket_port(self):
        console_type = self.get_console_type()
        websocket_port = utils.get_xml_path(self._XMLDesc(0), "/domain/devices/graphics[@type='%s']/@websocket" % console_type)
        return websocket_port

    def get_console_passwd(self):
        return utils.get_xml_path(self._XMLDesc(VIR_DOMAIN_XML_SECURE), "/domain/devices/graphics/@passwd")
    ...
    def get_console_keymap(self):
        return utils.get_xml_path(self._XMLDesc(VIR_DOMAIN_XML_SECURE), "/domain/devices/graphics/@keymap") or ''
    ...
    def get_snapshot(self):
        ...
        for snapshot in snapshot_list:
            ...
            snap_create_time = utils.get_xml_path(snap.getXMLDesc(0), "/domainsnapshot/creationTime")
            snap_state = utils.get_xml_path(snap.getXMLDesc(0), "/domainsnapshot/state")
            snapshots.append(
                {'name': snapshot, 'state': snap_state,
    ...
    def get_vm_storage(self, pool):
        return KvmStoragePool(self.host, self.login, self.passwd, self.conn, pool)

    def get_vm_storages(self):
        return KvmStorageBase(self.host, self.login, self.passwd, self.conn)
        ...
        for disk in tree.findall('devices/disk'):
            if disk.get('device') == 'disk':
                ...
                if source_file:
                    ...
                    vol_format = utils.get_xml_path(vol.XMLDesc(0), "/volume/target/format/@type")
                ...
                if source_dev:
                    ...
                    vol_name = utils.get_xml_path(vol.XMLDesc(0), "/volume/name")
                    pool_name = utils.get_xml_path(stg.XMLDesc(0), "/pool/name")

                    storage = self.get_wKvmStorage(pool_name)
    ...
    def get_guest_agent(self):
        ...
        return utils.get_xml_path(self._XMLDesc(0), func=_get_agent)
