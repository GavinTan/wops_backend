...
class soloConnection(object):
    ...
    def __init__(self, host, login=None, passwd=None, conn=2):
...
class KvmConnection:
    ...
    """
    def __init__(self, host, login, passwd, conn_type):
        """
...
class KvmConnectionManager:
    ...
    def get_connection(self, host, login, passwd, conn_type):
        """
        ...
        connection = self._search_connection(host, login, passwd, conn_type)
        ...
        if connection is None:
            ...
            try:
                ...
                if connection is None:
                    ...
                    connection = KvmConnection(host, login, passwd, conn_type)
...
connection_manager = KvmConnectionManager(
...
class KvmBase:
    def __init__(self, host, login, passwd, conn_type, conn_pool=True):
        # self.login = login
        # self.host = host
        # self.passwd = passwd
        # self.type = conn_type
        ...
        if conn_pool:
            ...
            self.wvm = KvmConnectionManager().get_connection(host, login, passwd, conn_type)
        else:
            self.wvm = soloConnection(host, login, passwd, conn_type).connect()
    ...
    def get_capabilities(self, arch):
        ...
        return utils.get_xml_path(self.get_cap_xml(), func=guests)
    ...
    def get_dom_capabilities(self, arch, machine):
        ...
        result["path"] = utils.get_xml_path(xml, "/domainCapabilities/path")
        result["domain"] = utils.get_xml_path(xml, "/domainCapabilities/domain")
        result["machine"] = utils.get_xml_path(xml, "/domainCapabilities/machine")
        result["vcpu_max"] = utils.get_xml_path(xml, "/domainCapabilities/vcpu/@max")
        result["iothreads_support"] = utils.get_xml_path(xml, "/domainCapabilities/iothreads/@supported")
        result["os_support"] = utils.get_xml_path(xml, "/domainCapabilities/os/@supported")

        result["loader_support"] = utils.get_xml_path(xml, "/domainCapabilities/os/loader/@supported")
        ...
        result["disk_support"] = utils.get_xml_path(xml, "/domainCapabilities/devices/disk/@supported")
        ...
        result["graphics_support"] = utils.get_xml_path(xml, "/domainCapabilities/devices/graphics/@supported")
        if result["graphics_support"] == 'yes':
            result["graphics_types"] = self.get_graphics_types(arch, machine)

        result["video_support"] = utils.get_xml_path(xml, "/domainCapabilities/devices/video/@supported")
        if result["video_support"] == 'yes':
            result["video_types"] = self.get_video_models(arch, machine)

        result["hostdev_support"] = utils.get_xml_path(xml, "/domainCapabilities/devices/hostdev/@supported")
        ...
        result["features_gic_support"] = utils.get_xml_path(xml, "/domainCapabilities/features/gic/@supported")
        result["features_genid_support"] = utils.get_xml_path(xml, "/domainCapabilities/features/genid/@supported")
        result["features_vmcoreinfo_support"] = utils.get_xml_path(xml, "/domainCapabilities/features/vmcoreinfo/@supported")
        result["features_sev_support"] = utils.get_xml_path(xml, "/domainCapabilities/features/sev/@supported")
    ...
    def is_kvm_supported(self):
        ...
        """
        return utils.is_kvm_available(self.get_cap_xml())
    ...
    def get_hypervisors_domain_types(self):
        ...
        return utils.get_xml_path(self.get_cap_xml(), func=hypervisors)
    ...
    def get_hypervisors_machines(self):
        ...
        return utils.get_xml_path(self.get_cap_xml(), func=machines)

    def get_emulator(self, arch):
        """
        :return: emulator list
        """
        return utils.get_xml_path(self.get_cap_xml(), "/capabilities/guest/arch[@name='{}']/emulator".format(arch))
    ...
    def get_emulators(self):
        ...
        return utils.get_xml_path(self.get_cap_xml(), func=emulators)
    ...
    def get_os_loaders(self, arch='x86_64', machine='pc'):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_os_loaders)
    ...
    def get_os_loader_enums(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_os_loader_enums)
    ...
    def get_disk_bus_types(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_bus_list)
    ...
    def get_disk_device_types(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_device_list)
    ...
    def get_graphics_types(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_graphics_list)
    ...
    def get_cpu_modes(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_cpu_modes)
    ...
    def get_cpu_custom_types(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_custom_list)
    ...
    def get_hostdev_modes(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_hostdev_list)
    ...
    def get_video_models(self, arch, machine):
        ...
        return utils.get_xml_path(self.get_dom_cap_xml(arch, machine), func=get_video_list)
    ...
    def get_storage(self, name, path=False):
        ...
        if path:
            path = utils.get_xml_path(pool.XMLDesc(), "/pool/target/path")
    ...
    def get_network_forward(self, net_name):
        def get_forward(doc):
            forward_mode = utils.get_xpath(doc, '/network/forward/@mode')
            return forward_mode or 'isolated'

        net = self.get_network(net_name)
        xml = net.XMLDesc(0)
        return utils.get_xml_path(xml, func=get_forward)
    ...
    def get_net_devices(self):
        ...
        def get_info(doc):
            dev_type = utils.get_xpath(doc, '/device/capability/@type')
            interface = utils.get_xpath(doc, '/device/capability/interface')
            return dev_type, interface

        for dev in self.wvm.listAllDevices(0):
            xml = dev.XMLDesc(0)
            (dev_type, interface) = utils.get_xml_path(xml, func=get_info)
            if dev_type == 'net':
    ...
    def get_host_instances(self, raw_mem_size=False):
        ...
        def get_info(doc):
            mem = utils.get_xpath(doc, "/domain/currentMemory")
            mem = int(mem) // 1024
            if raw_mem_size:
                mem = int(mem) * (1024 * 1024)
            cur_vcpu = utils.get_xpath(doc, "/domain/vcpu/@current")
            if cur_vcpu:
                vcpu = cur_vcpu
            else:
                vcpu = utils.get_xpath(doc, "/domain/vcpu")
            title = utils.get_xpath(doc, "/domain/title")
            title = title if title else ''
            description = utils.get_xpath(doc, "/domain/description")
        ...
        for name in self.get_instances():
            ...
            xml = dom.XMLDesc(0)
            (mem, vcpu, title, description) = utils.get_xml_path(xml, func=get_info)
            vname[dom.name()] = {
                'status': dom.info()[0],
                'uuid': dom.UUIDString(),
                'vcpu': vcpu,
    ...
    def get_user_instances(self, name):
        ...
        def get_info(ctx):
            mem = utils.get_xpath(ctx, "/domain/currentMemory")
            mem = int(mem) // 1024
            cur_vcpu = utils.get_xpath(ctx, "/domain/vcpu/@current")
            if cur_vcpu:
                vcpu = cur_vcpu
            else:
                vcpu = utils.get_xpath(ctx, "/domain/vcpu")
            title = utils.get_xpath(ctx, "/domain/title")
            title = title if title else ''
            description = utils.get_xpath(ctx, "/domain/description")
            description = description if description else ''
            return mem, vcpu, title, description
    ...
    def find_uefi_path_for_arch(self, arch, machine):
        ...
        patterns = utils.UEFI_ARCH_PATTERNS.get(arch)
    ...
    def label_for_firmware_path(self, arch, path):
        ...
        for arch, patterns in utils.UEFI_ARCH_PATTERNS.items():
            for pattern in patterns:
                if re.match(pattern, path):
    ...
    def arch_can_uefi(self, arch):
        ...
        """
        return arch in list(utils.UEFI_ARCH_PATTERNS.keys())
